            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="pull-right">
                        <a class="btn btn-success" href="{{ url('admin/categories/subcats/create/'.$data['user']->id) }}"> Create New  Sub category</a>
                    </div>
                    ...
                    <table id="datatable" class="table table-striped ">
                        ...
                        @php
                            $i=0;
                        @endphp
                        @foreach ($data['user']->childs as $key => $user)
                            @php
                                $i++;
                            @endphp
                            <tr>
                                <td>{{$i}}</td>
                                <td>{{$user->name}} </td>
                                {{--<td><a href="{{ url('admin/categories/subcats/'.$user->id) }}">--}}

                                        {{--<button class="btn btn-success waves-effect waves-light" type="submit">Sub Categories<span class="badge badge-pill badge-primary float-right">{{$user->childs()->count()}}</span></button>--}}

                                    {{--</a></td>--}}

                                <td>
                                    <a href="{{ url('admin/categories/subcats/statusUpdate/'.$user->id) }}">
                                        @if($user->status==1)
                                ...
                                <td>
                                    <a href="{{ url('admin/categories/featuredUpdate/'.$user->id) }}">
                                        @if($user->featured==1)
                                ...
                                <td>
                                    <a class="btn btn-primary" href="{{ url('admin/categories/subcats/edit/'.$user->id) }}"><i class="fas fa-edit"></i></a>
                                    <form id = "sub_form" class="form-horizontal" action="{{url('admin/categories/subcats/destroy/'.$user->id)}}" method="POST" enctype="multipart/form-data"  >
                                    {{csrf_field()}}
