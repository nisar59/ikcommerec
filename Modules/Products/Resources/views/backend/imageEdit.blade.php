        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="card-title">Update {{$data['product']->name}}'s Images </h4>
                    {{--<p class="card-title-desc">DataTables has most features enabled by--}}
                    {{--default, so all you need to do to use it with your own tables is to call--}}
                    {{--the construction function: <code>$().DataTable();</code>.--}}
                    {{--</p>--}}

                    {!! Form::model($data['user'], ['method' => 'POST','url' => ['admin/products/images/update/'.$data['user']->id],"enctype"=>"multipart/form-data"]) !!}
                    <div class="row">


                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="form-group">
                            @if(!empty($data['user']->images))
                                    <img src="{{url('public/uploads/products/'.$data['user']->images)}}" width="100">
                                        <br>
                            @endif
