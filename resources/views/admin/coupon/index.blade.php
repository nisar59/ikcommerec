        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <a class="btn btn-success" href="{{ url('admin/coupon/create') }}"> Create New Coupon</a>
                    </div>
                    ...
                    <table id="datatable" class="table table-striped ">
                        ...
                        @foreach ($data['orders'] as $key => $user)
                            ...
                            <tr>
                                ...
                                <td>{{$user->name}}</td>
                                <td>{{$user->code}} </td>
                                <td>{{getDiscountTypeById($user->discount_type)}} </td>
                                <td>{{getDiscountForById($user->discount_for)}} </td>
                                <td>{{$user->discount}} </td>
                                ...
                                <td>
                                        ...
                                        <a class="btn btn-primary" href="{{ url('admin/coupon/edit/'.$user->id) }}"><i class="fas fa-edit"></i></a>

                                    <form id = "sub_form" class="form-horizontal" action="{{url('admin/coupon/destroy/'.$user->id)}}" method="POST" enctype="multipart/form-data"  >
                                    {{csrf_field()}}
